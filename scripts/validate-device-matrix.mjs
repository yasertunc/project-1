...
const DEVICE_MATRIX_WORKFLOW = join(rootDir, '.github/workflows/device-matrix.yml');
const DEVICE_MATRIX_DOC = join(rootDir, 'docs/mobile-device-matrix.md');
const DEVICE_MATRIX_IMPL_DOC = join(rootDir, 'docs/mobile-device-matrix-implementation.md');
...
function checkWorkflowFile() {
  ...
  try {
    ...
    const checks = [
      { name: 'Workflow triggers configured', pattern: /on:\s*[\s\S]*push|schedule/i },
...
function checkDocumentation() {
  ...
  const docs = [
    ...
    { path: DEVICE_MATRIX_IMPL_DOC, name: 'Device matrix implementation guide' },
  ];
