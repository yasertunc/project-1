...
function generateReport() {
  ...
  if (report.files && report.files.length > 0) {
    ...
    report.files.forEach((file) => {
      const fileStats = {
        ...
        failed: file.tests?.filter(t => t.outcome === 'unexpected').length || 0,
        skipped: file.tests?.filter(t => t.outcome === 'skipped').length || 0,
      };
      
      const status = fileStats.failed > 0 ? '❌' : fileStats.passed > 0 ? '✅' : '⏭️';
      const fileName = file.file.replace(process.cwd(), '').replace(/\\/g, '/');
      
      console.log(`${status} ${fileName}`);
      console.log(`   Total: ${fileStats.total} | Passed: ${fileStats.passed} | Failed: ${fileStats.failed} | Skipped: ${fileStats.skipped}`);
    });
  }
  ...
  if (report.files) {
    report.files.forEach((file) => {
      ...
      if (fileName.includes('flow') && !fileName.includes('api') && !fileName.includes('failure')) {
        categories.flow += file.tests?.length || 0;
      } else if (fileName.includes('failure') || fileName.includes('api-failures')) {
  ...
  if (stats.flaky > 0) {
    console.log('🔄 Some tests are flaky. Consider adding retries or fixing timing issues.');
  }
  
  if (passRate < 80) {
    console.log('📉 Pass rate is below 80%. Review test failures and fix issues.');
  } else if (passRate === 100) {
    console.log('🎉 All tests passed! Great job!');
  }
  
  if (stats.skipped > 0) {
    console.log(`⏭️  ${stats.skipped} tests were skipped. Consider running them when UI is ready.`);
  }
