...
function checkErrorBoundaryImplementation() {
  ...
  try {
    ...
    const checks = [
      { name: 'Class component extends React.Component', pattern: /class.*ErrorBoundary.*extends.*React\.Component/ },
...
function checkHomepageErrorBoundaries() {
  ...
  try {
    ...
    const checks = [
      ...
      { name: 'Force error utility declared', pattern: /__FELLOWUS_FORCE_SECTION_ERROR/ },
      { name: 'ErrorBoundary used for lazy sections', pattern: /<ErrorBoundary/ },
      { name: 'SectionGuard component', pattern: /SectionGuard|shouldForceSectionError/ },
    ];
