...
const CHECKLIST_ITEMS = [
  {
    ...
    items: [
      {
        id: "internal-testing",
        label: "Internal testing track validated",
        check: () => true,
      }, // Manual check
      {
        id: "no-p0-bugs",
        label: "No P0/P1 bugs in internal testing",
        check: () => true,
      }, // Manual check
      {
        id: "privacy-policy",
        label: "Privacy policy published",
        check: () => existsSync("public/privacy/index.html"),
      },
      {
        id: "terms-of-service",
        label: "Terms of service published",
        check: () => existsSync("public/terms/index.html"),
      },
      {
        id: "store-listing",
        label: "Store listing content ready",
        check: () => true,
      }, // Manual check
      {
        id: "support-channels",
        label: "Support channels established",
        check: () => true,
      }, // Manual check
    ],
  },
  {
    ...
    items: [
      {
        id: "open-testing-track",
        label: "Open testing track created",
        check: () => true,
      }, // Manual check
      {
        id: "release-notes",
      ...
      {
        id: "store-listing-beta",
        label: "Store listing with beta disclaimer",
        check: () => true,
      }, // Manual check
      {
        id: "content-rating",
        label: "Content rating completed",
        check: () => true,
      }, // Manual check
      {
        id: "rollout-percentage",
        label: "Initial rollout percentage set (10-20%)",
        check: () => true,
      }, // Manual check
    ],
  },
  {
    ...
    items: [
      {
        id: "public-link",
        label: "Public TestFlight link created",
        check: () => true,
      }, // Manual check
      {
        id: "beta-app-review",
        label: "Beta App Review passed",
        check: () => true,
      }, // Manual check
      {
        id: "test-information",
        label: "Test information provided",
        check: () => true,
      }, // Manual check
  ...
  },
  {
    ...
    items: [
      {
        id: "sentry-alerts",
        label: "Sentry alerts configured for beta",
        check: () => true,
      }, // Manual check
      {
        id: "analytics-setup",
      ...
      {
        id: "crash-reporting",
        label: "Crash reporting active",
        check: () => true,
      }, // Manual check
      {
        id: "feedback-mechanism",
        label: "Feedback collection mechanism ready",
        check: () => true,
      }, // Manual check
    ],
  },
    ...
    items: [
      {
        ...
        label: "Beta announcement prepared",
        check: () => true,
      }, // Manual check
      {
        id: "support-email",
      ...
      }, // Manual check
      {
        ...
        label: "Community channels ready",
        check: () => true,
      }, // Manual check
    ],
  },
...
function runChecklist() {
  ...
  for (const category of CHECKLIST_ITEMS) {
    ...
    for (const item of category.items) {
      ...
      const isManual =
        item.check.toString().includes("Manual check") ||
        item.check.toString().includes("true");
