...
function checkAppConfig() {
  ...
  try {
    ...
    const checks = [
      {
        name: "iOS bundle identifier configured",
        pattern: /bundleIdentifier/i,
      },
      ...
      {
        name: "Privacy description (tracking)",
        pattern: /NSUserTrackingUsageDescription/i,
      },
    ];
      ...
      if (profiles[profile]) {
        ...
        if (profileConfig.ios) {
          ...
          if (
            ...
            profileConfig.ios.simulator === false
          ) {
...
function checkFirebaseIOS() {
  ...
  if (existsSync(googleServices)) {
    ...
    console.log(
      `  ⏸️  iOS: GoogleService-Info.plist needed when iOS app added to Firebase`
    );
  } else {
...
function checkGitHubSecrets() {
  ...
  const requiredSecrets = ["ASC_API_KEY_ID", "ASC_ISSUER_ID", "ASC_API_KEY_P8"];
