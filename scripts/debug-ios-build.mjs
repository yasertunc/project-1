...
function checkPackageJson() {
  ...
  try {
    ...
    log(`  ✅ React Native: ${pkg.dependencies['react-native'] || 'not found'}`);
    
    // Check for problematic dependencies
    const problematic = [];
    if (pkg.dependencies.react && !pkg.dependencies.react.startsWith('19.1')) {
      problematic.push(`React version: ${pkg.dependencies.react} (should be 19.1.0)`);
    }
...
function getLatestBuildInfo() {
  ...
  try {
    const output = execSync('npx eas-cli build:list --platform ios --limit 1 --non-interactive', {
      cwd: MOBILE_DIR,
      stdio: 'pipe',
      encoding: 'utf-8',
    });
...
function generateReport(results) {
  ...
  if (failed > 0) {
    ...
    console.log('  3. Check build logs: npx eas-cli build:list --platform ios --limit 1');
    console.log('  4. Try: npx eas-cli build --platform ios --profile production-ios --clear-cache');
  } else {
    console.log('\n✅ All checks passed!');
    console.log('\n💡 If build still fails:');
    console.log('  1. Check build logs in Expo Dashboard');
    console.log('  2. Try: npx eas-cli build --platform ios --profile production-ios --clear-cache');
