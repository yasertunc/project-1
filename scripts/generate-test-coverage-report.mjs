...
function analyzeTestFile(filePath) {
  try {
    ...
    const hasRouteMocking =
      content.includes("route") || content.includes("intercept");

    // Check for assertions
    const hasAssertions =
      content.includes("expect(") || content.includes("toHave");
...
function generateReport(analyses) {
  ...
  const totalDescribes = analyses.reduce(
    (sum, a) => sum + (a.describeCount || 0),
    0
  );
  ...
  console.log(
    `  Route Mocking: ${withRouteMocking}/${analyses.length} files (${Math.round((withRouteMocking / analyses.length) * 100)}%)`
  );
  console.log(
    `  Assertions: ${withAssertions}/${analyses.length} files (${Math.round((withAssertions / analyses.length) * 100)}%)`
  );
  console.log(
    `  Error Handling: ${withErrorHandling}/${analyses.length} files (${Math.round((withErrorHandling / analyses.length) * 100)}%)`
  );
